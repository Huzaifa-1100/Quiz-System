...
const api = "https://opentdb.com/api.php?amount=6&category=18&difficulty=easy&type=multiple";
...
let startQuiz = async () => {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        if (askQuiz.quiz == computerQuiz[i].correct_answer) {
            ...
            console.log(chalk.bgRed.white(`Wrong, Correct Ans is ${computerQuiz[i].correct_answer}`));
            console.log("-".repeat(40));
        }
    }
    console.log("-".repeat(60));
    console.log(chalk.bgWhite.red(`\tDear ${chalk.green(userName.name)}, your score is ${chalk.green(score)} out of ${chalk.green("5")}`));
    if (score >= 3 && score < 5) {
        console.log(chalk.bgYellow.red(`Dear ${userName.name} Congratulations You Got Passing Marks`));
    }
    else if (score == 5) {
        console.log(chalk.bgWhite.red(`\tDear ${userName.name}, Congratulations on a Perfect Score`));
    }
    else {
